...
int CACEplayerDlg::UpdateINI(char *errStr)
{
	...
	GetDlgItemText(IDC_AUX, auxread, sizeof(auxread));
	CHEC(printfINI(errStr, fname, "AUX PROCESSING", "%s", auxread))
...
void CACEplayerDlg::DisplayInitScreen()
{
	...
	char drive[16], dir[256], ext[8], errStr[256], fname[256], buffer[256], buf[64];
	FulfillFile(fname, AppPath, INIFILE);
	...
	sprintfFloat(dres, 1, buf, sizeof(buf));
	SetDlgItemText(IDC_LEVEL, buf);
...
{	// out: seq, resStr
	...
	const char *fullname;
	char str[MAX_PATH*2], buf[MAX_PATH*20], verstr[64], fname[MAX_PATH], fext[MAX_PATH], *errstr, *warnstr, errStr[512];
	...
	if (wavdata.GetNextChan()==NULL) // MONO file
		{
			if (param->ptParam2!=NULL) // multi processing. 2-ch wave file (with duplicate monos) is created.
			{
				...
				if (!wavdata.Wavwrite(str, buf)) { resStr += buf; return 0;}
	...
	res = GateProcess(seq, buf, fullname, streamFile, param, &errstr, &warnstr);
