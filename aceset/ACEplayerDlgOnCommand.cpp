...
void CACEplayerDlg::OnCommand(int idc, HWND hwndCtl, UINT event)
{
	...
	char fname[MAX_PATH], fullfname[MAX_PATH], estr[256], str[256];
	ACESEQ seq;
	...
	case IDC_WAVE_LIST:
		...
		if (event == LBN_DBLCLK) // Prepare and Play (stream) current file
		{
			...
			sprintf(str, "WAV=wave(\"%s\")", fullfname);
			try {
				AstSig.Reset(FS);
				AstSig.SetNewScript(str);
				AstSig.Compute();
			...
			if (PrepareAUX())
			{
				...
				//signal ready. 
				if (!Sig.Wavwrite(str, estr))
				{
					MessageBox (estr);
					break;
				}
				::SendMessage (hStatusbar, SB_SETTEXT, 1, (LPARAM)"Preparing");
				if (!Prepare(&seq, str, duration, outStr))
				{
