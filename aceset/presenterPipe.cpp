...
{
	char str[256], buf[256], buf1[64], buf2[64], buf3[64], buf4[64], buf5[64];
	char big[65536], med[4096];

	sprintfFloat(level, 1, buf, sizeof(buf));
	...
	sprintf(out+strlen(out), ", %s/%d, level=%s dBSPL RMS, Q=%d, max=%d\r\n", getPwStr(param,str), (int) param->pg
		, buf, param->Q, param->numMax);
	big[0]=0;
	for (int i=0; i<22; ++i) {
		if (param->selected[i]) {
			sprintfFloat(param->t[i], 1, buf1, sizeof(buf1));
			sprintfFloat(param->c[i], 1, buf2, sizeof(buf2));
			sprintfFloat(param->gains[i], 1, buf3, sizeof(buf3));
			sprintfFloat(param->freqsL[i], 1, buf4, sizeof(buf4));
			sprintfFloat(param->freqsU[i], 1, buf5, sizeof(buf5));
			sprintf(med,"%-5d%-6s%-6s%6s%6s  %s\r\n", 22-i, buf4, buf5, buf1, buf2, buf3);
			strcat(big, med);
		}
	}
	if (strlen(param->addmlines)>0) {
		sprintf(med,"Additional p fields: %s\r\n", param->addmlines);
		strcat(big, med);
	}
	if (strlen(out)+strlen(big)+1 > outsize) {
		strncat(out, big, outsize - strlen(out) - 4);
		strcat(out, "...");
	} else
		strncat(out, big, outsize - strlen(out) - 1);
}
