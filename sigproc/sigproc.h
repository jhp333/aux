...
public:
	...
	EXP_CS void *PlayArray(int DevID, UINT userDefinedMsgID, HWND hApplWnd, double *block_dur_ms, char *errstr); // playing with event notification by specified time block
	EXP_CS void *PlayArray(int DevID, UINT userDefinedMsgID, HWND hApplWnd, int nProgReport, char *errstr); // full format
...
public:
	...
	EXP_CS void *Play(int devID, char *errstr, int len);
