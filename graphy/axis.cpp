...
CAxis::CAxis(CGobj* pParent /*=NULL*/)
{
	strcpy(type, "axis");
...
void CAxis::DeleteLine(int index)
{
	int j=0, i=0;
	int res=1;
	if (index==-1)
...
void CAxis::setRange(const char xy, double x1, double x2)
{
	if (x2<x1) return;
	if (xy=='x' || xy=='X')
	{
...
void CAxis::setTick(const char xy, double const x1, double const x2, int const nDiv, int const nSkips, char const *format, double const coeff, double const offset)
{
	if (xy=='x' || xy=='X')
	{
		xtick.a1=x1;
...
CLine * CAxis::plot(int length, double *y, COLORREF col, char cymbol, int lineWidth)
{
	int i;
	CLine * out;
...
CLine * CAxis::plot(int length, double *x, double *y, COLORREF col, char cymbol, int lineWidth)
{
	int i;
	...
	for (i=0; i<nLines; i++)
		tempLine[i] = m_ln[i];
	tempLine[nLines]=new CLine(this, length);
	delete[]  m_ln;
	m_ln  = tempLine;
