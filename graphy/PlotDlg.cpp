...
CPlotDlg::CPlotDlg(const CSignals &data, CWnd* pParent /*=NULL*/)
: CDialog(CPlotDlg::IDD, pParent), levelView(false), specView(false), devID(0)
{
	//{{AFX_DATA_INIT(CPlotDlg)
		// NOTE: the ClassWizard will add member initialization here
	//}}AFX_DATA_INIT
	sig = data;
	gcf = new CFigure(this);
...
BEGIN_MESSAGE_MAP(CPlotDlg, CDialog)
	...
	ON_WM_KEYDOWN()
	//}}AFX_MSG_MAP
	ON_COMMAND_RANGE(IDM_FULLVIEW, IDM_SPECTRUM, OnMenu)
	ON_WM_LBUTTONUP()
END_MESSAGE_MAP()
...
CRect CPlotDlg::DrawAxis(CDC *pDC, CAxis *pax)
{
	...
	pDC->FillSolidRect(&rt2, pax->colorBack);
	pDC->MoveTo(org);
...
void CPlotDlg::OnRButtonUp(UINT nFlags, CPoint point) 
{
	...
	if (hax!=NULL)
	{
		...
		ClientToScreen(&point);    // To convert point to re: the whole screen 
...
void CPlotDlg::OnMenu(UINT nID)
{
	...
	case IDM_PLAY:
		...
		else if (!signal.PlayArray(devID, errstr))	MessageBox (errstr);
		break;
	case IDM_SPECTRUM:
	...
	case IDM_SPECTRUM_INTERNAL:
		...
		if (multi) 
		{
			...
			maxx = 5.*(maxmag/5.+1);
			for (int i=0; i<len/2; i++)	mag2[i] -= maxx;
		}
	...
	case IDM_WAVWRITE:
		...
		strcpy(szFilter, "Wav Files (*.wav)|*.wav|All Files (*.*)|*.*||");
		CFileDialog dllg(0, "wav", NULL, OFN_HIDEREADONLY, szFilter);
		dllg.DoModal();
		CString fname = dllg.GetFileName();
		if (fname.GetLength()>0)
			if (!signal.Wavwrite(fname, errstr))	MessageBox (errstr);
		return;
	}
	InvalidateRect(NULL);
}
...
BOOL CPlotDlg::OnCommand(WPARAM wParam, LPARAM lParam)
{
